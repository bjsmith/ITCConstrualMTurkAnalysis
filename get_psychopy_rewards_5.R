...
get_psychopy_rewards_5 <- function(data.directory,metaloop_dir,task_version,raw.filedir,condition_info){#raw.filedir<-raw.psychopy.filedir
  ...
  for (rfn in raw.filenames){#rfn<-raw.filenames[1]#rfn<-"YGFIWQQF__Thu_Mar_15_2018_112759_GMT-0800_AKDT_107.77.205.115.csv"
    ...
    #if we find a qualtricsresponseID under qualtricsresponseid then use that (third row)
    if("qualtricsresponseid" %in% colnames(pruned_csv)) qualtricsrid<-trimws(pruned_csv$qualtricsresponseid[!is.na(pruned_csv$qualtricsresponseid)][3])#first two rows are practice, don't take them seriously.
    #if we find a qualtricsresponseID under qualtricsresponseid1 then use that (first row)
    if("qualtricsresponseid1" %in% colnames(pruned_csv)) qualtricsrid<-trimws(pruned_csv$qualtricsresponseid1[!is.na(pruned_csv$qualtricsresponseid1)][3])#first two rows are practice, don't take them seriously.
    ...
    if(is.na(subid)){
      subid<-pruned_csv$qualtricssid1[!is.na(pruned_csv$qualtricssid1)][3]#this might exist if the first try didn't work well.
    }
    ...
    if(condition_info$Condition[condition_info$ConditionCode==conditionCode]=="Hypothetical"){
      ...
      if(doubleTrial){
        ...
        #includedInDrawer=rep(pruned_csv$runid==pruned_csv$RewardRun,2) & trimws(responseCol)!="" & rep(pruned_csv$PracticeRun==FALSE,2)
        includedInDrawer=rep(pruned_csv$runid==pruned_csv$RewardRun,2) & rep(pruned_csv$PracticeRun==FALSE,2)
      }else{
        ...
        includedInDrawer=pruned_csv$runid==pruned_csv$RewardRun & pruned_csv$PracticeRun==FALSE
      }
      ...
      rewardedRow=choosecol==min(choosecol[includedInDrawer],na.rm = TRUE) & includedInDrawer & !is.na(choosecol)
      if(doubleTrial){
      ...
      if(trimws(responseCol[rewardedRow])==""){
        rewardDelay=0
        rewardAmount=0.2#the minimum amount we'll payout to subjects who missed a round (we are forgiving!)
        subjChoice=responseCol[rewardedRow]
      }else if (responseCol[rewardedRow]=="SS"){
    ...
    subjListEntry<-data.frame("SubID"=subid,"ConditionCode"=conditionCode,
                              "QualtricsResponseID"=qualtricsrid,
                              "RewardDelay"=rewardDelay,
                              "PsychoPyDate"=pruned_csv$date[1],#as.POSIXlt(substr(pruned_csv$date[1],6,16),format="%b %d %Y"),
