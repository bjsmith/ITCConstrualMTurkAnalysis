...
get_psychopy_rewards_3 <- function(data.directory,metaloop_dir,task_version,raw.filedir,condition_info){#raw.filedir<-raw.psychopy.filedir
  ...
  for (rfn in raw.filenames){#rfn<-raw.filenames[1]#rfn<-"YGFIWQQF__Thu_Mar_15_2018_112759_GMT-0800_AKDT_107.77.205.115.csv"
    ...
    if(condition_info$Condition[condition_info$ConditionCode==conditionCode]=="Hypothetical" & 
      ...
      rewardedRow=choosecol==max(choosecol[includedInDrawer],na.rm = TRUE) & includedInDrawer & !is.na(choosecol)
      #print(which(rewardedRow))
